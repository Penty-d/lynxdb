...
func init() {
	...
	pf.StringVarP(&globalFormat, "format", "F", "auto", "Output format: auto, json, ndjson, table, box, ascii, markdown, vertical, line, G, csv, tsv, raw")
	...
	pf.IntVar(&globalMaxRows, "max-rows", 0, "Maximum rows to show in human table output (0 = all)")
	pf.StringVar(&globalNullValue, "null-value", "", "Placeholder for null or empty values in human table output")
	pf.IntVar(&globalMaxWidth, "max-width", 0, "Maximum width for human table output (0 = terminal width)")
