...
func TestLintQuery_DoubleQuotedNames(t *testing.T) {
	tests := []struct {
		...
		{
			name:      "bin command",
			query:     `from app | bin "duration ms" span=100 as "duration bucket"`,
			wantCodes: []string{LintDoubleQuotedName, LintDoubleQuotedName},
		},
		{
			name:      "join command",
		...
		{
			name:      "sort command",
			query:     `from app | sort -"duration ms", +"status code"`,
			wantCodes: []string{LintDoubleQuotedName, LintDoubleQuotedName},
		},
		{
			name:      "order by command",
			query:     `from app | order by "duration ms" desc`,
			wantCodes: []string{LintDoubleQuotedName},
		},
		{
		...
		{
			name:      "select command",
			query:     `from app | select "user id" as "display name"`,
			wantCodes: []string{LintDoubleQuotedName, LintDoubleQuotedName},
		},
		{
			name:      "keep command",
