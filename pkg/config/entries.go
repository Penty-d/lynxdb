...
func EntriesWithCLI(configPath string, cli []CLIOverride) []Entry {
	...
	add("storage.compaction_disable_bsi_on_output", strconv.FormatBool(cfg.Storage.CompactionDisableBSIOnOutput), strconv.FormatBool(dflt.Storage.CompactionDisableBSIOnOutput), strconv.FormatBool(fileCfg.Storage.CompactionDisableBSIOnOutput), LYNXDB_STORAGE_COMPACTION_DISABLE_BSI_ON_OUTPUT)
	...
	add("query.broad_source_lint_threshold", strconv.Itoa(cfg.Query.BroadSourceLintThreshold), strconv.Itoa(dflt.Query.BroadSourceLintThreshold), strconv.Itoa(fileCfg.Query.BroadSourceLintThreshold), LYNXDB_QUERY_BROAD_SOURCE_LINT_THRESHOLD)
	add("query.broad_segment_lint_threshold", strconv.Itoa(cfg.Query.BroadSegmentLintThreshold), strconv.Itoa(dflt.Query.BroadSegmentLintThreshold), strconv.Itoa(fileCfg.Query.BroadSegmentLintThreshold), LYNXDB_QUERY_BROAD_SEGMENT_LINT_THRESHOLD)
	...
	add("ingest.splunk_hec.enabled", strconv.FormatBool(cfg.Ingest.SplunkHEC.Enabled), strconv.FormatBool(dflt.Ingest.SplunkHEC.Enabled), strconv.FormatBool(fileCfg.Ingest.SplunkHEC.Enabled), LYNXDB_INGEST_SPLUNK_HEC_ENABLED)
	add("ingest.splunk_hec.require_token", strconv.FormatBool(cfg.Ingest.SplunkHEC.RequireToken), strconv.FormatBool(dflt.Ingest.SplunkHEC.RequireToken), strconv.FormatBool(fileCfg.Ingest.SplunkHEC.RequireToken), LYNXDB_INGEST_SPLUNK_HEC_REQUIRE_TOKEN)
	...
	add("ingest.staging.max_bytes", cfg.Ingest.Staging.MaxBytes.String(), dflt.Ingest.Staging.MaxBytes.String(), fileCfg.Ingest.Staging.MaxBytes.String(), LYNXDB_INGEST_STAGING_MAX_BYTES)
	add("ingest.staging.max_age", cfg.Ingest.Staging.MaxAge.String(), dflt.Ingest.Staging.MaxAge.String(), fileCfg.Ingest.Staging.MaxAge.String(), LYNXDB_INGEST_STAGING_MAX_AGE)
	add("ingest.staging.max_inflight_events", strconv.Itoa(cfg.Ingest.Staging.MaxInflightEvents), strconv.Itoa(dflt.Ingest.Staging.MaxInflightEvents), strconv.Itoa(fileCfg.Ingest.Staging.MaxInflightEvents), LYNXDB_INGEST_STAGING_MAX_INFLIGHT_EVENTS)
	add("ingest.staging.flush_retries", strconv.Itoa(cfg.Ingest.Staging.FlushRetries), strconv.Itoa(dflt.Ingest.Staging.FlushRetries), strconv.Itoa(fileCfg.Ingest.Staging.FlushRetries), LYNXDB_INGEST_STAGING_FLUSH_RETRIES)
	add("ingest.staging.flush_backoff_max", cfg.Ingest.Staging.FlushBackoffMax.String(), dflt.Ingest.Staging.FlushBackoffMax.String(), fileCfg.Ingest.Staging.FlushBackoffMax.String(), LYNXDB_INGEST_STAGING_FLUSH_BACKOFF_MAX)
	add("ingest.mode", cfg.Ingest.Mode, dflt.Ingest.Mode, fileCfg.Ingest.Mode, LYNXDB_INGEST_MODE)
