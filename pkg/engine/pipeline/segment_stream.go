...
func (s *SegmentStreamIterator) matchesStreamSourceScope(segIndex string) bool {
	...
	case "glob":
		if h.SourceScopePattern != "" {
			return glob.Match(h.SourceScopePattern, segIndex, false)
		}
	}
	...
	if h.SourceGlob != "" {
		...
		return glob.Match(h.SourceGlob, segIndex, false)
	}
...
//
// TERM leaf: search for the single term.
...
func evaluateTermTree(tree *model.SearchTermTree, idx *index.SerializedIndex) *roaring.Bitmap {
	if tree == nil {
		return nil
	}
...
func filterEventsByTimeBounds(events []*event.Event, tb *model.TimeBounds) []*event.Event {
...
// BuildRGFilter constructs a segment.RGFilterNode tree from SegmentStreamHints.
// Returns nil when no filterable predicates exist in the hints.
//
// The resulting tree enables the RGFilterEvaluator to perform unified
// per-row-group pruning (const columns, presence, zone maps, per-column blooms).
func BuildRGFilter(hints *SegmentStreamHints) *segment.RGFilterNode {
	...
	// Search terms -> bloom filter term nodes.
	...
	// Range predicates.
	// We always use >= for Min and <= for Max to match the segment reader's
	// predicate conversion in NewSegmentStreamIterator. The RG filter must not
	// be stricter than the row-level filter applied during ReadRowGroupFiltered.
	for _, rp := range hints.RangePreds {
		if rp.Min != "" {
			children = append(children, segment.RGFilterNode{
				Op:       segment.RGFilterFieldRange,
...
// normalizeField converts user-facing field names to physical column names.
// For example, "source" -> "_source", "sourcetype" -> "_sourcetype".
