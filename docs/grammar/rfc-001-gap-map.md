...
| Advanced multivalue eval functions | Deferred | Basic `split`, `mvappend`, `mvcount`, `mvdedup`, and `mvjoin` execute and are suggested in eval contexts; deferred multivalue functions are omitted from eval completion catalogs because `mvfind`, `mvindex`, `mvmap`, `mvrange`, `mvsort`, `mvzip`, `mv_to_json_array`, and `json_array_to_mv` need a native multivalue representation or explicit compatibility contract. |
