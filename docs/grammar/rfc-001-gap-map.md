...
| Aggregate aliases `mean`, `median`, `distinct_count`, `estdc`, `p50`, `p75`, `p90`, `p95`, `p99`, and supported percentile forms normalize to executable aggregate functions | `pkg/spl2/parser.go`, `pkg/spl2/parser_test.go`, `pkg/api/rest/server_test.go` |
...
| Eval functions `like`, `validate`, `searchmatch`, `nullif`, `in`, `printf`, `ipmask`, `toint`, `todouble`, `tobool`, `isbool`, `isarray`, `isobject`, `typeof`, `replace`, `split`, `trim`, `ltrim`, `rtrim`, `urldecode`, `md5`, `sha1`, `sha256`, `sha512`, `strftime`, `strptime`, `log`, `exp`, `pow`, `pi`, `random`, trig/hyperbolic functions, variadic `max`, and variadic `min` compile and execute through VM opcodes and are present in hint/editor catalogs | `pkg/vm/compiler.go`, `pkg/vm/vm.go`, `pkg/vm/vm_test.go`, `pkg/vm/compiler_test.go`, `pkg/spl2/error_hints.go`, `web/src/editor/lynxflow-catalog.ts` |
...
| Math eval functions | Splunk docs define `log(x)` as base 10, `log(x, base)` as explicit-base logarithm, `ln(x)` as natural logarithm, `exp(x)` as e raised to x, `pow(x, y)` as exponentiation, `pi()` as a no-argument constant, and `random()` as a pseudo-random integer from 0 to 2^31-1. |
...
| `searchmatch` eval function | Splunk docs define `searchmatch(search_str)` as returning true when the current event matches the supplied search string. |
...
| Function catalog | Many eval and aggregate functions parse and execute; common aggregate aliases, time aggregates, RFC conversion aliases, `printf`, `ipmask`, `strftime`/`strptime`, conditional helpers, scalar and JSON-string type predicates, common string trim/encoding functions, cryptographic hash functions, common math/random and trig/hyperbolic functions including variadic eval `max`/`min`, and `like()` now normalize or execute before planning | RFC aggregate/eval catalog still needs a full parser, VM, and editor cross-check for remaining functions and aliases. |
...
| CLI/TUI assistance | Shell autocomplete command, eval function, JSON function, and aggregate function vocabularies are sourced from the parser hint catalog; `eval`/`where` contexts suggest supported eval functions; `lynxdb query --no-lint` passes `lint:false`; `lynxdb query --no-suggestions` passes `suggestions:false` for server-mode CLI, TUI, and `--queries-file` requests; server-mode CLI/TUI results render returned lints on stderr; `lynxdb query --show-rewritten` renders normalizer rewrites on stderr with a 4 KiB preview cap | Shell query-context autocomplete is still less rich than web autocomplete for snippets, templates, ranking reasons, and per-request `meta.suggestions` UX. |
...
| `sigfig(num)` eval precision | Deferred | Splunk `sigfig` depends on significant-figure metadata from the originating calculation or literal formatting; current `event.Value` stores numeric magnitude but not decimal precision provenance. |
| Aggregate `span`, `sparkline`, `dataset()`, and `pivot()` | Deferred | Current aggregate execution covers scalar, distinct, percentile, variance, list, values, mode, first/last, earliest/latest, and rate families; these functions need histogram bucket metadata, sparkline series values, row-object arrays, or key/value pivot state distinct from existing aggregate outputs. |
| Native array/object eval conversions | Deferred | `isarray` and `isobject` work for valid JSON string values; `toarray`, `tomv`, `toobject`, and `tojson` need native structured value support or a documented JSON-string conversion contract. |
| Stateful time eval functions `now()`, `time()`, and `relative_time()` | Deferred | `strftime` and `strptime` execute and are suggested in eval contexts; deferred stateful time functions are omitted from eval completion catalogs because `now()` needs search-start time, `time()` needs per-event evaluation time, and `relative_time()` needs the same calendar-aware relative-time model as calendar `M`/`y` units. |
| Advanced multivalue eval functions | Deferred | Basic `split`, `mvappend`, `mvcount`, `mvdedup`, and `mvjoin` execute and are suggested in eval contexts; deferred multivalue functions are omitted from eval completion catalogs because `mvfilter`, `mvfind`, `mvindex`, `mvmap`, `mvrange`, `mvsort`, `mvzip`, `mv_to_json_array`, and `json_array_to_mv` need a native multivalue representation or explicit compatibility contract. |
| SPL2 JSON exact/path mutation aliases | Deferred | Current JSON execution covers path-based extract, keys, array length, object/array creation, type, set, remove, and merge; `json_extract_exact`, `json_set_exact`, `json_append`, `json_extend`, and `json_delete` need exact-key and path-value-pair semantics distinct from the existing path helpers. |
