...
| Full duration grammar including calendar `M`/`y` units and `@w0`/`@w1` snap variants | Deferred | Current parser and runtime cover signed `s`/`m`/`h`/`d`/`w` relative ranges; calendar-aware units and week-start snap variants need a time arithmetic model beyond `time.Duration`. |
