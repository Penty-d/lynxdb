...
| Eval functions `like`, `validate`, `searchmatch`, `nullif`, `in`, `printf`, `ipmask`, `toint`, `todouble`, `tobool`, `isbool`, `isarray`, `isobject`, `typeof`, `trim`, `ltrim`, `rtrim`, `urldecode`, `md5`, `sha1`, `sha256`, `sha512`, `strftime`, `strptime`, `log`, `exp`, `pow`, `pi`, `random`, trig/hyperbolic functions, variadic `max`, and variadic `min` compile and execute through VM opcodes and are present in hint/editor catalogs | `pkg/vm/compiler.go`, `pkg/vm/vm.go`, `pkg/vm/vm_test.go`, `pkg/vm/compiler_test.go`, `pkg/spl2/error_hints.go`, `web/src/editor/lynxflow-catalog.ts` |
...
| Stateful time eval functions `now()`, `time()`, and `relative_time()` | Deferred | `strftime` and `strptime` execute; `now()` needs search-start time, `time()` needs per-event evaluation time, and `relative_time()` needs the same calendar-aware relative-time model as calendar `M`/`y` units. |
| Advanced multivalue eval functions | Deferred | Basic `split`, `mvappend`, `mvcount`, `mvdedup`, and `mvjoin` execute using the current multivalue string representation; `mvfind`, `mvindex`, `mvmap`, `mvrange`, `mvsort`, `mvzip`, `mv_to_json_array`, and `json_array_to_mv` need a native multivalue representation or explicit compatibility contract. |
