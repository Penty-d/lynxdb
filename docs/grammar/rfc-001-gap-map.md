...
| Rewrite transparency | `NormalizeQueryWithRewrites` records structured `Rewrite{Before, After, Reason}` entries for normalizer rewrites | REST/TUI/CLI metadata paths are not fully wired to surface `meta.rewrites` and `--show-rewritten`. |
