...
| Time modifiers | `index=... earliest=... latest=...` normalizes to compact source time ranges, including `now`/`now()` latest values; `index=... _index_earliest=... _index_latest=...` normalizes to explicit `_indextime` predicates; lossless deprecated `daysago`, `hoursago`, `minutesago`, and matching `end*` forms normalize to compact ranges | `starttime`, `endtime`, and `timeformat` still need format-aware compatibility handling. |
